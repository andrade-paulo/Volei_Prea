...
import { TeamBoard, TeamHeader, TeamRoster } from "~/components/progress/teams";

type TeamSide = 0 | 1;

export function meta({}: Route.MetaArgs) {
  return [{ title: "Seleção manual — Vôlei Preá" }];
}

export default function ManualTeams() {
  const navigate = useNavigate();
  const { players, addPlayer, getPlayerName, makeManualTeams, startMatch } = useProgressData();
  const [teams, setTeams] = useState(() => makeManualTeams());
  ...
  function beginMatch() {
    startMatch("manual", teams[0], teams[1]);
